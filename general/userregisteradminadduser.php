...
if(isset($_SESSION['adminname'])){
  echo " hello admin : " . $_SESSION['adminname'];
}
else{
  echo "hello new user";
}
...
if(isset($_POST['userreg']) && isset($_POST['userfirstname']) && isset($_POST['userlastname']) && isset($_POST['useremail']) && isset($_POST['userpassword'])){
 ...
 // $_POST['userage'] = 1;
  ...
  $inputcheckarray = array( "userfirstname"=>$userfirstname,
  ...
  if($userage<4){
    ...
    if($_SESSION['flashmessage'] == ""){                            
      ...
      echo "wea are coming here";
      $datainput = [$_POST['username']];
      $res = $conn->prepare($sqlcheckusername);
      $res->execute($datainput);
      
      if($res->rowCount() == 0){
        $sqlinsert = "INSERT INTO `phpcrudappdb`.`usertable`(`userfirstname`,`userlastname`,`username`,`userage`,`useremail`,`usergender`,`userimage`,`userdob`,`userpassword`) VALUES(?,?,?,?,?,?,?,?,?);";
        $inputdata = [$userfirstname,$userlastname,$username,$userage,$useremail,$usergender,$userimage,$userdob,$userpassword];
...
<body>
  <form action="<?= $_SERVER['PHP_SELF'] ?>" method="post">
    ...
    <label>email</label> <input type="email" name="useremail" id="user" value="<?php if(isset($_SESSION['tempuseremail'])){echo $_SESSION['tempuseremail'];}else{echo '';} ?>" required></br>
